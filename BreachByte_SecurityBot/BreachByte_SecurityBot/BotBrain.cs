...
namespace BreachByte_SecurityBot
{
    public class BotBrain
    {
        ...
        //Gets or sets the user's saved name for personalised responses.
        public string SavedUserName { get; set; } = "User";

        //Gets or sets the user's favourite cybersecurity topic.
        public string FavoriteTopic { get; set; } = "";

        //Tracks the last topic discussed to support follow-up questions
        public string LastDiscussedTopic { get; set; } = "";

        //Signals to MainWindow which topic was just triggered, for activity logging
        public string TopicJustTriggered { get; set; } = "";

        //The knowledge base dictionary mapping topic keywords to response arrays.
        ...
        //Initialises the BotBrain and populates the knowledge base.
        ...
        public BotBrain()
        {
            ...
            knowledgeBase.Add("safe browsing", new string[]
            {
                ...
                "Frequently clear browser cache, and avoid using download managers as they can introduce malware.",
                ...
                "Familiarize yourself with privacy policies to understand data sharing practices, and refrain from interacting with pop-ups, using 'alt + F4' instead of clicking close buttons." // (Massachusetts College of Pharmacy and Health Sciences, n.d.)
            });
            ...
            knowledgeBase.Add("banking scams", new string[]
            {
                ...
                "\nTo keep it simple, cybercrimes in banking involve illegal activities that exploit technology to target financial institutions and their customers. " +
                ...
                "and to report any suspected fraud to their bank immediately." // (Standard Bank, n.d.)
            });
            ...
            knowledgeBase.Add("malware and ransomware", new string[]
            {
                ...
                "Ransomware typically entails hackers encrypting an organization's files to prevent access. After that, they demand payment to grant access to the data. " +
                "In recent times, several forms of cyber extortion, such as information theft, have been referred to as ransomware.", // (ICO, n.d.)
            ...
            knowledgeBase.Add("identity theft", new string[]
            {
                ...
                "Thieves can obtain information from your social media profiles [NAME]; or they can simply steal mail from your postbox or your dustbin. " +
                "In fact, any document, printed or electronic, that contains your name and any other personal information puts you at risk of identity theft.",
                ...
                "Ensure all your private correspondence stays private [NAME] - lock your postbox; don't throw old accounts away without first destroying them; " +
                "don't leave personal documents lying around where others could see them; and protect your online identity.", // (TransUnion, n.d.)
        ...
        /// Processes the user's input and returns an appropriate bot responses
        /// <param name="utext">The raw text input from the user.</param>
        /// <returns>A formatted response string.</returns>
        public string GetBotResponse(string utext)
        {
            ...
            if (input.Contains("my name is"))
            {
                ...
                if (parts.Length > 1)
                {
                    ...
                    // Strip away any punctuation attached to the name
                    extractedName = extractedName.Trim('.', ',', '!', '?');

                    // Capitalise and save it
            ...
            {
                // Grab the last word of the sentence as the topic
            ...
            if (input.Contains("tell me more") || input.Contains("another tip") || input.Contains("explain more") || input.Contains("what else can you tell me"))
            {
                ...
                {
                    // Re-use the last topic to fetch a new random response
                    input = LastDiscussedTopic;
                }
                else
                {
                    return "I'm not sure what you want to hear more about! Type 'What can i ask about' to see available cybercrime topics or type 'bye' to end our conversation😊.";
                }
            }

            // --- Knowledge Base Lookup ---

            // Reset trigger flag before each lookup
            TopicJustTriggered = "";

            foreach (var key in knowledgeBase.Keys)
            {
                // Split the dictionary key into individual words
                string[] keywords = key.ToLower().Split(' ');
                bool hasAllKeywords = true;

                // Check if EVERY keyword exists in the user's input
                ...
                // All keywords matched — trigger a response
                if (hasAllKeywords)
                {
                    ...
                    // Occasionally tie the response back to the user's favourite topic
            ...
            // --- Fallback: sentiment expressed but no topic matched ---
